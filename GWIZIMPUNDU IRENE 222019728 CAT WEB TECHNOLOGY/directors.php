...
<section>
<h1>Directors Form</h1>

    <form method="post" onsubmit="return confirmInsert();">
        <label for="Director_Id">Director_Id:</label>
        <input type="number" id="dus_Id" name="dus_Id"><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO directors (Director_Id, Names, Telephone, Email) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("isss", $Director_Id, $Names, $Telephone, $Email);

    // Set parameters from POST data with validation (optional)
    $Director_Id = intval($_POST['dus_Id']); // Ensure integer for ID
    $Names = htmlspecialchars($_POST['Ft_Nm']); // Prevent XSS
    $Telephone = htmlspecialchars($_POST['phn']); // Prevent XSS
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of directors</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of directors</h2></center>
    <table border="5">
        <tr>
            <th>Director_Id</th>
            <th>Names</th>
        ...
        // Prepare SQL query to retrieve directors.
        $sql = "SELECT * FROM directors";
        $result = $connection->query($sql);
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $did = $row['Director_Id']; // Fetch the Director_Id
                echo "<tr>
                    <td>" . $row['Director_Id'] . "</td>
                    <td>" . $row['Names'] . "</td>
                    <td>" . $row['Telephone'] . "</td>
                    <td>" . $row['Email'] . "</td>
                    <td><a style='padding:4px' href='delete_directors.php?Director_Id=$did'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_directors.php?Director_Id=$did'>Update</a></td> 
                </tr>";
            }
        } else {
